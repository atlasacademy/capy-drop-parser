        ...
        for currency in currencies:
            if drop['id'] == currency['id']:
                ...
                if not checkValueStringTop(top_stack_size_string):
                    ...
                    logging.info(f"{drop_identifier} | 2nd try top   : {top_stack_size_string}")

                if checkValueStringTop(top_stack_size_string):
                    drop['stack'] = get_stack_base_top(top_stack_size_string)
                    logging.info(f"{drop_identifier} | Top    string : {drop['stack']}")
                else:
                    logging.info(f'{drop_identifier} | Failed to get top stack count')
                    ...
                    logging.info(f"{drop_identifier} | 1st try bottom: {stack_size_string}")
                    ...
                    if not checkValueString(stack_size_string):
                        ...
                        logging.info(f"{drop_identifier} | 2nd try bottom: {stack_size_string}")

                    if checkValueString(stack_size_string):
                        drop['stack'] = get_stack_base(stack_size_string)
                        logging.info(f"{drop_identifier} | Bottom string : {drop['stack']}")
                    else:
...
def extract_text_from_image(image, file_name='pytesseract_input.png'):
    ...
    return pytesseract.image_to_string(qp_image, config='-l eng --oem 1 --psm 7')
